...
/*  function add_user_pads(username, pad_id){
  ...
  app.post('/save_pad', function(request, response) {
    ...
    pg.connect(process.env.DATABASE_URL, function(err, client, done) {
      save_or_update_pad_query = 'UPDATE pad SET last_modified_timestamp = NOW() ,content =  \'' + escape(chimpad_pad_content) + '\' ,last_modified_user = '+ chimpad_pad_user + ' WHERE id = ' + chimpad_pad_id + ';';
